...
# ToDo: check that no critical parameter is strictly dependent on existence of data in a specific pixel
#       (e.g., "len(det[0].chi" or similar))
...
#
# FUNCTION to 'combine' 100 pixels into one multi-element detector
...
#
# FUNCTION to correct dead time ("tau") for each detector pixel
#
...
#
# FUNCTION to skip parts of the mda ASCII header (find string "search")
#   returns: "line", which is the last string read in before exiting the loop
#
...
#
# FUNCTION to skip parts of mda ASCII header (find line starting with "search")
#   returns: "line", which is the last string read in before exiting the loop
#
...
#
# FUNCTION to normalise the individual, dead-time corrected spectra ("roiCorr") to I0
...
def getWeightFactors( det, e, e0, goodPixels ) :
    ...
    # also need an array to export later (will be used by the GUI to dispaly in pixel arrays
    ...
    k = np.sqrt(2 * m_e * (x-e0)*ee ) / hbar * 1e-10           # k in A^-1 for the postEdge region
    det[0].k = k    #  this is a bit of a cheap hack; we need to make "k" available
    #                   to the GUI; here we use the first detector Pixel Object
    #                   as a vessel to transport "k" from this module to the GUI "sakura[...].py";
    #                   this is not elegant, but it works
    for i in goodPixels :
        yPost = np.ndarray.flatten(det[i].roiCorr[ startIndexPost:stopIndexPost ])  # mu(E') above edge
        ...
        w = (postEdgeCurve[e0Index] - preEdgeCurve[e0Index]) / preEdgeCurve[e0Index]
        weights[i] = w
        #
        # while we have the fit in memory, extract something like "chi(k)" and
        #    write into detector Pixel Objects for use in the GUI
        postE = postEdgeCurve[ startIndexPost:stopIndexPost ]   # fit(E') only above edge
...
#
# FUNCTION to apply weight factors to the individual spectra
...
#
# FUNCTION to estimate which edge position (E0) might be most adequate
#   returns: e0
def getE0 (e):
    #
    # use simple approach:
    ...
    for k in etab.edgeEnergy.keys() :
        ...
        for j in range(len(energyList)) :
            if energyList[j] > eMin and energyList[j] < eMax and j < 4:
                ...
                shells.append( etab.QNTransition[j] )   # get corresponding shell and put into list 
                edgeEnergies.append( energyList[j])     # get corresponding edge energy and put into list
    ...
    for i in range(len(result)) :    # loop over subarrays;
        ...
        temp = []                    # empty out temp for next iteration in loop 'i'
    
    return result


#
# FUNCTION to compute the average of all weighted, deadtime corrected spectra
#    (from det[i].weightedSpec, which is = det[i].roiCorr * det[i].weightFactor)
#	returns: no explicit value, but generates a Class attributes "self.averageMu" "[...]Chi"
#
...
#
# FUNCTION to write a 2D-array of floats as 2D-array of strings into a file
#   input: open file instance (f), 2D-array to convert (output), float format (numFormat)
...
#
# FUNCTION to produce an output file containing averaged data and meta-information
...
def writeAverages(mdaOutName, goodPixels, correls,
    ...
    f.write('# ----- \n')
    f.write('# \n')
    f.write('# E[eV]    mu(E)_fluo_average/I0[a.u.]    I0[cts/sec]    I1[cts/sec]    I2[cts/sec]    sample_time[sec]    encoder_Bragg_angle[deg] \n')
    #
    output = []
    output = np.append(e,averageMu)
    output = np.append(output,trans[1:])   # energy axis already used (e = trans[0])
