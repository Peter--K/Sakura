...
class SakuraBaseMainFrame( sakura_gui_autogenerated.MainFrame ):
    ...
    def OnLeftDownPixel(self, event):
        ...
        if self.m_toggleBtn_MuChi.GetValue() == True:  # if toggled, display chi(k)
            ...
            y = self.results[self.specSelec].det[index].roiCorr
            xlabel = 'E  /  eV'
            ylabel = 'mu(E)*d  /  a.u.'
...
#             self.fname = dialog.GetFilename()
    ...
    def plotSpectrum(self, x, y, xtitle, ytitle, canvas, moreY=None, vlines=None,
        ...
        if vlines is not None:
            ...
            if vline_ymax is not None:
                ...
                ymax = vline_ymax
            for x in vlines:
                line_x = plot.PolyLine([(x,1), (x,ymax)], colour='green', width=2)
    ...
    def _read_data(self, whichFileToProcess):
