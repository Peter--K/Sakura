...
class SakuraBaseMainFrame( sakura_gui_autogenerated.MainFrame ):
    def __init__( self, parent ):
        self.detector_rows = 10
        self.detector_cols = 10
...
#         canvas.canvas.Bind(wx.EVT_LEFT_DOWN, self.OnLeftDown_SpectrumPlotCanvas)
#         canvas.canvas.Bind(wx.EVT_PAINT, self.OnPaint_SpectrumPlotCanvas)
#         canvas.canvas.Bind(wx.EVT_LEFT_UP, self.OnLeftUp_SpectrumPlotCanvas)
#         canvas.canvas.Bind(wx.EVT_MOTION, self.OnMouseMove_SpectrumPlotCanvas)
        # Hook up the wx.EVT_ERASE_BACKGROUND event to enable it to be ignored in order
        # to minimise flickering - see here:
        # http://stackoverflow.com/questions/6076014/force-repaint-in-wxpython-canvas
#         canvas.canvas.Bind(wx.EVT_ERASE_BACKGROUND, self.EraseCanvasBackground)
...
#         self.overlay = wx.Overlay()
        ...
        self.colourTable = np.loadtxt(os.path.join(os.path.dirname(__file__),
                                      'gist_heat.cmap'))
    ...
    def OnEnterPixel(self, event):
        ...
        if self.results[self.specSelec].goodPixels[index] != -2:
            ...
            canvasID = self.plotSpectrum(x, y, xlabel, ylabel, destination, moreY=moreY)
    ...
    def OnSetRoi(self, event):
        ...
        self.det.set_roi_limits(low, high)
        self._show_spectrum()        

    '''
    def OnLeftDown_SpectrumPlotCanvas(self, event):
        # Capture the mouse and save the starting position for the
        # rubber-band
        self.canvasSpectrum.selecting = True
        self.start_pos = event.GetPosition()[0]  # mouse x-pos
        # Draw the ROI selection using an overlay.
#         dc = wx.ClientDC(self.canvasSpectrum.canvas)
...
#         odc.Clear()
    ...
    def DrawOverlay(self, end_x):
        self.canvasSpectrum.Refresh()

        canvas_height = self.canvasSpectrum.canvas.ClientSize[1]
        start_pos = (self.start_pos, -1)
        end_pos = (end_x, canvas_height)
        rect = wx.RectPP(start_pos, end_pos)
    ...
    def OnMouseMove_SpectrumPlotCanvas(self, event):
        if not self.canvasSpectrum.selecting:
            return
        self.DrawOverlay(end_x=event.GetPosition()[0])

    def OnLeftUp_SpectrumPlotCanvas(self, event):
        self.canvasSpectrum.selecting = False
        self.start_pos = None
        self.ClearOverlay()
    '''
...
#     def EraseCanvasBackground(self, event):
#         # Swallow the wx.EVT_ERASE_BACKGROUND event to minimise flickering - see here:
#         # http://stackoverflow.com/questions/6076014/force-repaint-in-wxpython-canvas
#         pass
...
#             if not self.m_DetectorPanel.IsShown():
    ...
    def _focus_spectrum(self, dataset):
        ...
        if isinstance(dataset.det, gnc.Detector):
            ...
            wx.Notebook doesn't have a way to disable a tab. However, you can do the same
            thing by checking which tab is clicked on and if it is "disabled", veto the
            EVT_NOTEBOOK_PAGE_CHANGING or EVT_NOTEBOOK_PAGE_CHANGED event.
            '''
            self.canvasSpectrum.Clear()
    ...
    def _show_spectrum(self, index=None):
        ...
        if index is None:
            # TODO: fix this so that we pick up a stored index
            index = 0
    ...
    def plotSpectrum(self, x, y, xtitle, ytitle, canvas, moreY=None, vlines=None):
        ...
        vlines - an list of x-coordinates to place vertical (green) lines at extending
        ...
        if vlines is not None:
            ...
            ymax = 3000         # TODO: fix this maxval
            for x in vlines:
                line_x = plot.PolyLine([(x,0), (x,ymax)], colour='green', width=4)
    ...
    def set_roi_limits(self):
        ...
        # TODO: get rid of the next two lines after PVs are added to the mda file
        pv_dict['mca1.R0LO'] = ('long', '', [600])
        pv_dict['mca1.R0HI'] = ('long', '', [800])

        self.det.set_roi_limits(
            pv_dict['mca1.R0LO'][-1][0],
            pv_dict['mca1.R0HI'][-1][0],
